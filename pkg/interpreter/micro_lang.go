...
func (v *Interpreter) VisitExpr(ctx *parser2.ExprContext) interface{} {
	...
	if len(ctx.AllExpr()) == 2 && ctx.GetOp() != nil {
		...
		case parser2.MicroLangParserPLUS:
			result = left + right
		case parser2.MicroLangParserMINUS:
			result = left - right
		case parser2.MicroLangParserMULT:
			result = left * right
		case parser2.MicroLangParserDIV:
			result = left / right
		case parser2.MicroLangParserGREAT:
			if left > right {
				result = 1
			} else {
				result = 0
			}
		case parser2.MicroLangParserLESS:
			if left < right {
				result = 1
			} else {
				result = 0
			}
		case parser2.MicroLangParserEQUAL:
			if left == right {
				result = 1
			} else {
				result = 0
